// The Identity of A Producer — course workspace controller.
//
// Mounts into the shared /courses.html workspace ids (same as app.js +
// trust-process-app.js) but uses its own modules + store so progress and
// notes stay course-scoped.
//
// On full completion the status chip displays "Credential Earned" —
// the Course Completion Credential is granted by finishing all six modules.

import { store, saveNote } from './identity-producer-store.js';
import { MODULES, PILLARS } from './identity-producer-modules.js';
...
function updateTopBar() {
  ...
  const bc = $('breadcrumb');
  if (bc) bc.innerHTML = `<a href="/index.html" class="breadcrumb-link">Academy</a> / <a href="/courses.html" class="breadcrumb-link">Courses</a> / <a href="/courses.html?course=identity-producer" class="breadcrumb-link">Identity of A Producer</a> / <span>${escapeHtml(m.title)}</span>`;
  ...
  if (complete) {
    if (store.isComplete(store.currentModule)) {
      complete.textContent = isLast ? '✓ Credential Earned' : '✓ Completed — Next Module →';
      complete.className = 'btn btn-success';
      complete.onclick = () => navigate(1);
    } else {
      complete.textContent = isLast ? 'Complete Course & Earn Credential →' : 'Mark Complete & Continue →';
  ...
  // Relabel the shared "Certification" hero chip to "Credential" for this
  // course — completing all six modules earns the Course Completion Credential.
  const statusLabel = document.querySelector('#workspace-live-content .cert-status-label');
  if (statusLabel) statusLabel.textContent = 'Credential';
  ...
  if (statusEl) {
    if (totalDone === 0) {
      ...
      statusEl.textContent = 'Credential Earned!';
