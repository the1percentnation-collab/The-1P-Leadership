// The Money Story — course-scoped progress + notes store.
//
// Namespaced so nothing collides with 1P-CLC / Trust The Process / Identity:
//   localStorage state key:    money_story_state
//   localStorage notes prefix: ms_note_
//   Firestore progress docs:   users/{uid}/progress/ms_<id>
//   User doc fields:           msCurrentModule
//   Company member digest:     msCompletedCount / msCurrentModule
//
// No firestore.rules change needed — covered by existing wildcards.
...
const STATE_KEY = 'money_story_state';
const NOTE_PREFIX = 'ms_note_';
const PROGRESS_PREFIX = 'ms_';
  ...
  async load() {
    ...
    try {
      ...
      progressSnap.forEach((d) => {
        ...
        if (lid == null) return; // foreign course ids — ignore.
        hasAnyProgress = true;
        const data = d.data() || {};
      ...
      const currentFromUserDoc = userSnap.exists() && typeof userSnap.data().msCurrentModule === 'number'
        ? userSnap.data().msCurrentModule
        : null;
      ...
      if (!hasAnyProgress && local.completed.length) {
        ...
        Object.keys(remoteNotes).forEach((k) => { _notes[`ms_m_${k}`] = remoteNotes[k]; });
      }
      ...
      console.warn('[ms-store] remote load failed, falling back to localStorage:', e);
      this._loaded = true;
    }
  },
  ...
  async _migrateLocalToRemote(uid, local) {
    try {
      ...
      writes.push(setDoc(doc(db, 'users', uid), {
        msCurrentModule: local.current,
        lastActiveAt: serverTimestamp()
      }, { merge: true }));
      await Promise.all(writes);
    } catch (e) {
      console.warn('[ms-store] migration failed:', e);
    }
  },
  ...
  setCurrent(id) {
    ...
    if (user) {
      setDoc(doc(db, 'users', user.uid), {
        msCurrentModule: id,
        lastActiveAt: serverTimestamp()
      }, { merge: true }).catch((e) => console.warn('[ms-store] setCurrent remote write:', e));
  ...
  markComplete(id) {
    ...
    if (user) {
      setDoc(doc(db, 'users', user.uid, 'progress', docIdForLesson(id)), {
        completed: true,
        completedAt: serverTimestamp()
      }, { merge: true }).catch((e) => console.warn('[ms-store] markComplete remote write:', e));
      setDoc(doc(db, 'users', user.uid), {
        lastActiveAt: serverTimestamp(),
        msCurrentModule: this.currentModule
      }, { merge: true }).catch(() => {});
      this._mirrorMemberDigest(user);
    }
  },
  ...
  async _mirrorMemberDigest(user) {
    try {
      ...
      await setDoc(doc(db, 'companies', companyId, 'members', user.uid), {
        uid: user.uid,
        msCompletedCount: this.completed.size,
        msCurrentModule: this.currentModule,
        lastActiveAt: serverTimestamp()
      }, { merge: true });
    } catch (e) { /* best-effort */ }
...
export function saveNote(key, val) {
  ...
  //   ms_<id>         → whole-lesson note on progress/ms_<id>.notes
  //   ms_<id>_<slot>  → slotted note under progress/ms_<id>.noteSlots.<slot>
  const lessonMatch = key.match(/^ms_(\d+)(?:_(.+))?$/);
  ...
  if (sub) {
    setDoc(docRef, { noteSlots: { [sub]: val } }, { merge: true })
      .catch((e) => console.warn('[ms-store] saveNote sub remote write:', e));
  } else {
    setDoc(docRef, { notes: val }, { merge: true })
      .catch((e) => console.warn('[ms-store] saveNote remote write:', e));
  }
}
