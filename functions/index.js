...
async function notifyUser(db, recipientUid, notif, { typePrefKey } = {}) {
  ...
  const result = await db.runTransaction(async (tx) => {
    ...
    const patch = {
      ...
      // post or channel (course work reminders). Site-relative path.
...
async function runTick(db, { dryRun = false } = {}) {
  ...
  const [sequences, watches, reminders, courseReminders, launches] = await Promise.all([
  ...
  const steps = { sequences, watches, reminders, courseReminders, launches };
